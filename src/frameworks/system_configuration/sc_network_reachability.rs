...
fn SCNetworkReachabilityGetFlags(
    ...
    // Report a reachable network so games can proceed past startup checks.
    // Actual socket/HTTP access is still gated by --allow-network-access.
    let out_flags = if let Some(addr) = env
        .objc
        ...
        .address
    {
        ...
        // CreateWithName() and similar — assume general internet reachability.
        kSCNetworkReachabilityFlagsReachable
    };

    env.mem.write(flags, out_flags);
    log_dbg!(
        "SCNetworkReachabilityGetFlags({:?}, {:?}) -> true (flags {:#x}, network_access: {})",
        target,
        flags,
        out_flags,
