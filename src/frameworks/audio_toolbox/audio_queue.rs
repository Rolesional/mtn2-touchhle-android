...
use crate::frameworks::core_audio_types::{
    debug_fourcc, fourcc, kAudioFormatAppleIMA4, kAudioFormatFlagIsFloat, kAudioFormatFlagIsPacked,
    kAudioFormatFlagIsSignedInteger,
    kAudioFormatLinearPCM, kAudioFormatMPEGLayer3, kAudioFormatMPEG4AAC,
...
struct AudioQueueHostObject {
    ...
    /// Leftover bytes when decoding compressed (MP3/AAC) stream buffers.
...
pub fn AudioQueueNewOutput(
    ...
    format = normalize_audio_queue_format(env, format);
    ...
    if !is_supported_audio_format(&format) {
        ...
        log!(
            "Warning: Audio queue {:?} uses unsupported format: sample_rate={}, format_id={}, flags={:#x}, channels={}, bits={}, bytes_per_frame={}",
            aq_ref,
            sample_rate,
...
fn normalize_audio_queue_format(
    env: &Environment,
    mut format: AudioStreamBasicDescription,
) -> AudioStreamBasicDescription {
    // C&C Red Alert tags PCM data as '.mp3'.
    if env.bundle.bundle_identifier().starts_with("com.ea.candcra")
        && format.format_id == kAudioFormatMPEGLayer3
    {
        log!("Applying game-specific hack for C&C Red Alert: Fixing hardcoded audio format from .mp3 to PCM.");
        format = mp3_tagged_pcm_format(&format);
    }
    format
}

fn mp3_tagged_pcm_format(format: &AudioStreamBasicDescription) -> AudioStreamBasicDescription {
    AudioStreamBasicDescription {
        sample_rate: if format.sample_rate > 0.0 {
            format.sample_rate
        } else {
            44100.0
        },
        ...
        frames_per_packet: 1,
        bytes_per_frame: 4,
        channels_per_frame: if format.channels_per_frame > 0 {
...
pub fn is_supported_audio_format(format: &AudioStreamBasicDescription) -> bool {
    ...
    match format_id {
        ...
        kAudioFormatMPEGLayer3 | kAudioFormatMPEG4AAC => true,
        kAudioFormatLinearPCM => {
            // Big-endian PCM is handled during decoding (e.g. Resident Evil 4).
...
pub fn decode_buffer(
    ...
    compressed_pending: Option<&mut Vec<u8>>,
    ...
    match format.format_id {
        kAudioFormatMPEGLayer3 | kAudioFormatMPEG4AAC => {
            return decode_compressed_buffer(data_slice, format, compressed_pending);
        }
...
fn decode_compressed_buffer(
    ...
    let try_decode = |data: &[u8]| -> Option<(ALenum, ALsizei, Vec<u8>)> {
        let pcm = crate::audio::try_decode_chunk_to_pcm(data)?;
        let al_format = match pcm.channels {
            1 => al::AL_FORMAT_MONO16,
            2 => al::AL_FORMAT_STEREO16,
            channels => {
                log!("Unsupported channel count {channels} in compressed audio buffer");
                return None;
            }
        };
        Some((al_format, pcm.sample_rate as ALsizei, pcm.bytes))
    };

    if let Some(result) = try_decode(data_slice) {
        if let Some(pending) = compressed_pending {
            pending.clear();
        }
        return result;
    }
    ...
    if let Some(pending) = compressed_pending {
        pending.extend_from_slice(data_slice);
        if let Some(result) = try_decode(pending) {
            pending.clear();
            return result;
        }
        log_dbg!(
            "Buffered {} bytes of compressed audio, waiting for more data",
            pending.len()
        );
    }

    // Symphonia could not decode this chunk yet. Emit a short silence buffer so
    // playback can continue while more stream data arrives.
    let channels = format.channels_per_frame.max(2);
    let rate = if format.sample_rate > 0.0 {
        format.sample_rate as ALsizei
    } else {
        44100
    };
    let silence_frames = 1024usize;
    let silence = vec![0u8; silence_frames * channels as usize * 2];
    let al_format = if channels == 1 {
        al::AL_FORMAT_MONO16
    } else {
        al::AL_FORMAT_STEREO16
    };
    (al_format, rate, silence)
}

/// Ensure an audio queue has an OpenAL source and at least one queued OpenAL
/// buffer. Returns the number of audio frames prepared.
fn prime_audio_queue(env: &mut Environment, in_aq: AudioQueueRef) -> Result<u32, ()> {
    let (state, context) =
    ...
    loop {
        ...
        let compressed_pending = &mut host_object.compressed_pending;
        ...
        assert!(unsafe { context.GetError() } == 0);
        frames_prepared = frames_prepared.saturating_add(frames_in_decoded_buffer(
            &format,
            next_buffer.audio_data_byte_size,
            data.len(),
        ));
...
fn frames_in_decoded_buffer(
    format: &AudioStreamBasicDescription,
    input_byte_size: u32,
    decoded_pcm_byte_size: usize,
) -> u32 {
    match format.format_id {
        kAudioFormatAppleIMA4 => {
            let packets = input_byte_size / 34;
            packets * 64
        }
        kAudioFormatMPEGLayer3 | kAudioFormatMPEG4AAC => {
            let channels = format.channels_per_frame.max(2);
            (decoded_pcm_byte_size as u32) / (channels * 2)
        }
        kAudioFormatLinearPCM => {
            let bytes_per_frame = format.bytes_per_frame.max(1);
            input_byte_size / bytes_per_frame
        }
        _ => 0,
    }
}
...
fn AudioQueuePrime(
    ...
    match prime_audio_queue(env, in_aq) {
        ...
        Err(_) => {
            ...
            let format_flags = format.format_flags;
            log!(
                "Warning: Cannot prime audio queue! (format_id: {}, sample_rate: {}, channels: {}, flags: {:#x})",
                debug_fourcc(format.format_id),
                sample_rate,
                channels_per_frame,
                format_flags,
