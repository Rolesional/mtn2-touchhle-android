...
pub fn user_data_base_path() -> Cow<'static, Path> {
    ...
    unsafe {
        ...
        let path = if path.is_null() {
            log!("Couldn't get Android external storage path, trying internal storage");
            SDL_AndroidGetInternalStoragePath()
        } else {
            path
        };
        if path.is_null() {
            log!("Couldn't get Android storage path!");
            panic!();
        }
...
pub fn url_for_opening_user_data_dir() -> Result<String, String> {
    if std::env::consts::OS == "android" {
        ...
        Ok("content://com.expectional.mtn2.provider/root/root".to_string())
    } else {
