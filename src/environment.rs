...
enum ThreadNextAction {
    ...
    /// Return to host.
    ReturnToHost,
    /// Guest thread called pthread_exit (or returned via thread exit routine).
    ExitThread,
                ...
                match svc {
                    ...
                    dyld::Dyld::SVC_THREAD_EXIT => {
                        ...
                        if self.current_thread != 0 {
                            ...
                            curr_thread.return_value =
                                Some(mem::Ptr::from_bits(self.cpu.regs()[0]));
                            curr_thread.active = false;
                            log_dbg!(
                                "Thread {} exited via thread exit routine",
                                self.current_thread
                            );
                        } else {
                            log_dbg!("Main thread hit thread exit routine");
                        }
                        ThreadNextAction::ExitThread
                    }
                }
            }
        ...
        loop {
                ...
                .is_none_or(|remaining_ticks| remaining_ticks > 0)
            {
                ...
                match self.handle_cpu_state(state) {
                    ThreadNextAction::Continue => {}
                    ThreadNextAction::ReturnToHost | ThreadNextAction::ExitThread => return,
