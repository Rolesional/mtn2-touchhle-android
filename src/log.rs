...
pub fn get_log_file() -> &'static File {
    static LOG_FILE: LazyLock<File> = LazyLock::new(|| {
        let path = crate::paths::user_data_base_path().join("touchHLE_log.txt");
        File::options()
            .create(true)
            .append(true)
            .open(path)
            .unwrap_or_else(|e| {
                ...
                // stderr only if the log file can't be opened
                std::fs::File::create(std::env::temp_dir().join("touchHLE_log_fallback.txt"))
                    .unwrap_or_else(|_| {
                        std::fs::File::create("touchHLE_log_fallback.txt")
                            .expect("Couldn't open any log file")
                    })
            })
    });
