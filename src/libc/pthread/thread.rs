...
/// Terminate the calling thread. Real iOS libSystem routes this through
/// `_pthread_exit`, which eventually hits the thread exit SVC routine.
fn pthread_exit(env: &mut Environment, retval: MutVoidPtr) {
    log_dbg!("pthread_exit({:?}) on thread {}", retval, env.current_thread);
    env.cpu.regs_mut()[0] = retval.to_bits();
    env.cpu.branch(env.dyld.thread_exit_routine());
}

fn _pthread_exit(env: &mut Environment, retval: MutVoidPtr) {
    pthread_exit(env, retval);
}
